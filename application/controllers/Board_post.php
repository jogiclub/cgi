	...
	public function post($post_id = 0, $print = false)
	{
		...
		if (element('post_del', $post)) {
            ...
            if ($form && is_array($form)) {
                $extra_content = array(); // 배열 초기화
                $k = 0; // 카운터 초기화

                foreach ($form as $key => $value) {
                    if (empty($value) || ! element('use', $value)) {
                        continue;
                    }

                    // field_name이 없는 경우 건너뛰기
                    ...
                    $extravars = element('extravars', $post);

                    if ($extravars && is_array($extravars)) {
                        $item = element($field_name, $extravars);
                    }

                    $extra_content[$k] = array(
                        'field_name' => $field_name,
                        'display_name' => element('display_name', $value)
                    );

                    if (element('field_type', $value) === 'checkbox' && $item) {
                        try {
                            $tmp_value = json_decode($item, true);
                            $tmp = '';

                            if ($tmp_value && is_array($tmp_value)) {
                                $tmp = implode(', ', $tmp_value);
                            }
                            $item = $tmp;
                        } catch (Exception $e) {
                            log_message('error', 'JSON decode error in board post: ' . $e->getMessage());
                            $item = '';
                        }
                    }
	...
	public function _get_list($brd_key, $from_view = '')
	{
		...
		if ($noticeresult) {
            foreach ($noticeresult as $key => $val) {
                // extra_vars 가져오기
                $noticeresult[$key]['extra_vars'] = $this->Post_model->get_post_extra_vars(
                    element('post_id', $val),
                    element('brd_id', $board)
                );
		...
		if (element('list', $result)) {
            ...
            foreach (element('list', $result) as $key => $val) {
                // extra_vars 가져오기
                $result['list'][$key]['extra_vars'] = $this->Post_model->get_post_extra_vars(
                    element('post_id', $val),
                    element('brd_id', $board)
                );
        ...
        $return['data'] = $result;
        $return['notice_list'] = $noticeresult;  // 수정된 $noticeresult를 view로 전달

		...
		if ($form_validation === false OR $file_error) {
            ...
            if ($form && is_array($form)) {
                foreach ($form as $key => $value) {
                    ...
                    // item이 null인 경우를 대비한 안전한 처리
                    $item = '';
                    if (isset($post['extravars']) && is_array($post['extravars'])) {
                        $item = element(element('field_name', $value), element('extravars', $post));
                    }

                    $extra_content[$k] = array();  // 배열 초기화
                    $extra_content[$k]['field_name'] = element('field_name', $value);
                    $extra_content[$k]['display_name'] = element('display_name', $value);
                    $extra_content[$k]['input'] = '';
                    ...
                    if (element('field_type', $value) === 'text'
                        ...
                        if(element('options', $value)){
                            $options = explode("\n", element('options', $value));
                            $i = 1;
                            // item이 JSON 형태로 저장되어 있으므로 디코딩
                            $checkboxs = $item ? json_decode($item, true) : array();
                            foreach ($options as $okey => $oval) {
                                $oval = trim($oval);
                                $checked = is_array($checkboxs) && in_array($oval, $checkboxs);
                                $extra_content[$k]['input'] .= '<label for="' . element('field_name', $value) . '_' . $i . '"><input type="checkbox" name="' . element('field_name', $value) . '[]" id="' . element('field_name', $value) . '_' . $i . '" value="' . $oval . '" ' . set_checkbox(element('field_name', $value), $oval, $checked) . ' /> ' . $oval . ' </label> ';
                                $i++;
                            }
                        }
                        $extra_content[$k]['input'] .= '</div>';

...
class Mypage extends CB_Controller
{
    ...
    public function mycamp()
    {
        ...
        $result = $this->Camp_model->get_my_camp_list(
...
//        $view['view']['total_rows'] = $result['total_rows'];
        ...
        $config['base_url'] = site_url('mypage/mycamp') . '?' . $param->replace('page');
    ...
    public function update_camp() {
        ...
        if (!$this->member->is_member()) {
            $this->output->set_content_type('application/json')->set_output(json_encode(['success' => false, 'message' => '로그인이 필요합니다.']));
            return;
        }
        ...
        // 해당 신청이 현재 로그인한 사용자의 것인지 확인
        $this->load->model('Camp_model');
        $apply = $this->Camp_model->get_apply_one($idx);
//
//        print_r($this->member->item('mem_userid'));
//        exit;

        if (!$apply || $apply['wr_id'] != $this->member->item('mem_userid')) {
            $this->output->set_content_type('application/json')->set_output(json_encode(['success' => false, 'message' => '잘못된 접근입니다.']));
            return;
        }

        // 데이터 업데이트
        $result = $this->Camp_model->update_apply($idx, $data);

			...
			foreach (element('list', $result) as $key => $val) {
				...
				if (element('cqa_secret', $val)) {
					if ($mem_id && ($is_admin !== false OR (int) element('mem_id', $val) === $mem_id)) {
						$result['list'][$key]['content'] = '<div class="badge text-bg-warning">비밀글입니다</div> ' . $result['list'][$key]['content'];
						$result['list'][$key]['reply_content'] = '<div class="badge text-bg-warning">비밀글입니다</div>' . $result['list'][$key]['reply_content'];
					} else {
						$result['list'][$key]['content'] = '<div class="badge text-bg-warning">비밀글입니다</div>';
						$result['list'][$key]['reply_content'] = '<div class="badge text-bg-warning">비밀글입니다</div>';
					}
				}

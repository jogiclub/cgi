...
class Findaccount extends CB_Controller
{
	 ...
	 * 모델을 로딩합니다
	 */
    protected $models = array('Member_auth_email', 'Member');
	...
	function __construct()
	{
        ...
        $this->load->library(array('querystring', 'email', 'smslib')); // smslib 추가
	}
	...
	public function index()
	{
        ...
        } elseif ($this->input->post('findtype') === 'findsms') {
            $config[] = array(
                'field' => 'phone_number',
                'label' => '휴대폰번호',
                'rules' => 'trim|required|numeric|min_length[10]|max_length[11]|callback__existphone',
            );
		...
		if ($this->form_validation->run() === false) {
			...
			if ($this->input->post('findtype') === 'findidpw') {
                ...
                $this->email->initialize($this->_setup_email_config());
                ...
                // 이벤트가 존재하면 실행합니다
                $view['view']['event']['findsms_before'] = Events::trigger('findsms_before', $eventname);

                $phone = $this->input->post('phone_number');
                if (!isset($this->Member_model)) {
                    $this->load->model('Member_model');
                }

                $mb = $this->Member_model->get_by_phone($phone);
                if (!$mb) {
                    $this->load->model('Member_dormant_model');
                    $mb = $this->Member_dormant_model->get_by_phone($phone);
                }
                ...
                $content .= "어캠비번재설정:" . $verify_url;

                // SMS 발송
                $smsresult = $this->smslib->send($list, $sender, $content, '', '계정찾기');

                if ($smsresult) {
                    $view['view']['message'] = $phone . '로 계정정보가 전송되었습니다.';
                } else {
                    $view['view']['message'] = 'SMS 발송에 실패했습니다. 다시 시도해주세요.';
                }

                // 이벤트가 존재하면 실행합니다
                $view['view']['event']['findsms_after'] = Events::trigger('findsms_after', $eventname);
            }
    ...
    private function _setup_email_config() {
        $config = array(
            'protocol' => 'smtp',
            'smtp_host' => 'smtp.gmail.com',
            'smtp_port' => 587,
            'smtp_user' => 'jogiclub@gmail.com',
            'smtp_pass' => 'zdlt driw epud yoym',
            'smtp_crypto' => 'tls',
            'smtp_timeout' => 30,
            'newline' => "\r\n",
            'crlf' => "\r\n",
    ...
    private function generate_verification_code($length = 4) {
    ...
    public function _existphone($str)
    {
        ...
        $userinfo = $this->Member_model->get_by_phone($phone, 'mem_id, mem_denied');
        if (!$userinfo) {
            $this->load->model('Member_dormant_model');
            $userinfo = $this->Member_dormant_model->get_by_phone($phone, 'mem_id, mem_denied');
        }

        if (!element('mem_id', $userinfo)) {
            $this->form_validation->set_message('_existphone', '존재하지 않는 휴대폰번호입니다');
            return false;
        }

        if (element('mem_denied', $userinfo)) {
            $this->form_validation->set_message('_existphone', '회원님의 계정은 접근이 금지되어 있습니다');
            return false;
        }

...
class Post_model extends CB_Model
{
    ...
    public function get_notice_list($brd_id = 0, $except_all_notice = '', $sfield = '', $skeyword = '', $sop = 'OR')
    {
        ...
        $this->db->select('post.*, member.mem_id, member.mem_userid, member.mem_nickname, member.mem_icon, member.mem_photo, member.mem_point');
        $this->db->from($this->_table . ' as post');
        $this->db->join('member', 'post.mem_id = member.mem_id', 'left');

        if ($except_all_notice) {
            $this->db->where('post.brd_id', $brd_id);
            $this->db->where('post.post_notice', 1);
        } else {
            $this->db->where('(( post.brd_id = ' . $brd_id . ' AND post.post_notice = 1) OR post.post_notice = 2) ', null, false);
        }

        $this->db->where('post.post_del <>', 2);
        ...
        // 각 게시글의 extra_vars 데이터 가져오기
        if ($result) {
            foreach ($result as $key => $val) {
                $this->db->select('pev_key, pev_value');
                $this->db->from('post_extra_vars');
                $this->db->where('post_id', $val['post_id']);
                $this->db->where('brd_id', $brd_id);

                $extra_vars = $this->db->get()->result_array();

                // extra_vars 데이터를 연관 배열로 변환
                $result[$key]['extra_vars'] = array();
                foreach ($extra_vars as $extra) {
                    $result[$key]['extra_vars'][$extra['pev_key']] = $extra['pev_value'];
                }
            }
        }
    ...
    public function get_post_extra_vars($post_id, $brd_id)
    {
        if (empty($post_id) || empty($brd_id)) {
            return array();
        }
        ...
        foreach ($result as $extra) {
            $extra_vars[$extra['pev_key']] = $extra['pev_value'];
        }

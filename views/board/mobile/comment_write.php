...
?>
	...
	<button type="button" class="btn btn-secondary btn-sm admin-manage-comment"><i class="bi bi-cog big-fa"></i>댓글관리</button>
	<div class="btn-admin-manage-layer admin-manage-layer-comment">
		<div class="item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_delete', '0', '선택하신 글들을 완전삭제하시겠습니까?');"><i class="bi bi-trash-o"></i> 선택삭제</div>
		<div class="item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_secret', '0', '선택하신 글들을 비밀글을 해제하시겠습니까?');"><i class="bi bi-unlock"></i> 비밀글해제</div>
		<div class="item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_secret', '1', '선택하신 글들을 비밀글로 설정하시겠습니까?');"><i class="bi bi-lock"></i> 비밀글로</div>
		<div class="item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_blame_blind', '0', '선택하신 글들을 블라인드 해제 하시겠습니까?');"><i class="bi bi-exclamation-circle"></i> 블라인드해제</div>
		<div class="item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_blame_blind', '1', '선택하신 글들을 블라인드 처리 하시겠습니까?');"><i class="bi bi-exclamation-circle"></i> 블라인드처리</div>
		<div class="item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_trash', '', '선택하신 글들을 휴지통으로 이동하시겠습니까?');"><i class="bi bi-trash"></i> 휴지통으로</div>
	</div>
<?php
}
			...
			?>
				...
				<div class="comment_write_button_area">
					...
					<div class="btn-group pull-right" role="group" aria-label="...">
						...
						<?php } ?>
						<button type="button" class="btn btn-secondary btn-sm" title="새글등록" onClick="comment_box('', 'c');"><i class="bi bi-pencil fa-lg"></i></button>
						<button type="button" class="btn btn-secondary btn-sm" title="창을크게" onClick="resize_textarea('cmt_content', 'down');"><i class="bi bi-plus fa-lg"></i></button>
						<button type="button" class="btn btn-secondary btn-sm" title="창을작게" onClick="resize_textarea('cmt_content', 'up');"><i class="bi bi-minus fa-lg"></i></button>
					</div>
				</div>
				<?php if ($this->member->is_member() === false) { ?>
					<div class="form-inline passcord">
						...
						<?php } else { ?>
							...
							<div class="input-group">자동등록방지 숫자를 순서대로 입력하세요.</div>
						<?php } ?>
					</div>
				<?php } ?>
			<?php echo form_close(); ?>

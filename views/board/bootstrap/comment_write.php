...
?>
    <div class="row">
        <div class="col-sm-6">
            <div class="chk_comment_all_wrapper">
                <label for="chk_comment_all">
                    <input id="chk_comment_all" onclick="all_commentlist_checked(this.checked);" type="checkbox" /> 코멘트 전체선택
                </label>
            </div>
        </div>
        <div class="col-sm-6">
            <div class="input-group justify-content-end">
                <button type="button" class="btn btn-sm btn-secondary dropdown-toggle" data-bs-toggle="dropdown"><i class="bi bi-gear"></i> 관리</button>

                <ul class="dropdown-menu">
                    <li class="dropdown-item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_delete', '0', '선택하신 글들을 완전삭제하시겠습니까?');"><i class="bi bi-trash-o"></i> 선택삭제</li>
                    <li class="dropdown-item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_secret', '0', '선택하신 글들을 비밀글을 해제하시겠습니까?');"><i class="bi bi-unlock"></i> 비밀글해제</li>
                    <li class="dropdown-item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_secret', '1', '선택하신 글들을 비밀글로 설정하시겠습니까?');"><i class="bi bi-lock"></i> 비밀글로</li>
                    <li class="dropdown-item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_blame_blind', '0', '선택하신 글들을 블라인드 해제 하시겠습니까?');"><i class="bi bi-exclamation-circle"></i> 블라인드해제</li>
                    <li class="dropdown-item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_blame_blind', '1', '선택하신 글들을 블라인드 처리 하시겠습니까?');"><i class="bi bi-exclamation-circle"></i> 블라인드처리</li>
                    <li class="dropdown-item" onClick="comment_multi_action('viewcomment', '<?php echo element('post_id', element('post', $view)); ?>', 'comment_multi_trash', '', '선택하신 글들을 휴지통으로 이동하시겠습니까?');"><i class="bi bi-trash"></i> 휴지통으로</li>
                </ul>
            </div>
        </div>
    </div>
	...
	<div id="comment_write_box">
		<div class="well comment_write_box_inner">
			...
			?>
				...
				<?php } ?>
				<div class="comment_write_button_area mt-2 mb-2">
					<div class="input-group ">
						...
						<?php if (element('can_comment_secret', element('comment', $view))) { ?>
                            <div class="input-group-text">
								<label for="cmt_secret">
									<input class="form-check-input" type="checkbox" name="cmt_secret" id="cmt_secret" value="1" <?php echo set_checkbox('cmt_secret', '1', (element('cmt_secret', element('comment', $view)) ? true : false)); ?> /> 비밀글
								</label>
							</div>
						<?php } ?>
						...
						<?php if (element('use_emoticon', element('comment', $view))) { ?>
							<button type="button" class="btn btn-secondary btn-sm" title="이모티콘" onclick="window.open('<?php echo site_url('helptool/emoticon?id=cmt_content'); ?>', 'emoticon', 'width=600,height=400,scrollbars=yes')"><i class="bi bi-smile-o fa-lg"></i></button>
						<?php } ?>
						<?php if (element('use_specialchars', element('comment', $view))) { ?>
							<button type="button" class="btn btn-secondary btn-sm" title="특수문자" onclick="window.open('<?php echo site_url('helptool/specialchars?id=cmt_content'); ?>', 'specialchars', 'width=490,height=245,scrollbars=yes')"><i class="bi bi-star-o fa-lg"></i></button>
						<?php } ?>
<!--						<button type="button" class="btn btn-secondary btn-sm" title="새글등록" onClick="comment_box('', 'c');">새글등록</button>-->
						<button type="button" class="btn btn-secondary btn-sm" title="창을크게" onClick="resize_textarea('cmt_content', 'down');"><i class="bi bi-plus"></i>창크게</button>
						<button type="button" class="btn btn-secondary btn-sm" title="창을작게" onClick="resize_textarea('cmt_content', 'up');"><i class="bi bi-dash"></i></i>창작게</button>
					</div>
				</div>
				<?php if ($this->member->is_member() === false) { ?>
					<div class="form-inline passcord">
						...
						<?php } else { ?>
							<div class="input-group"><img src="<?php echo base_url('assets/images/preload.png'); ?>" width="160" height="40" id="captcha" alt="captcha" title="captcha" /></div>
							<div class="input-group">
								<input type="text" class="form-control col-md-4" id="captcha_key" name="captcha_key" />
							</div>
							<div class="input-group">자동등록방지 숫자입력</div>
						<?php } ?>
					</div>
				<?php } ?>
			<?php echo form_close(); ?>

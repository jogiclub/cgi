<div class="box-table">
		...
		<table class="table table-bordered table-hover table-striped">
			...
			?>
				<tr>
					...
					<td><button type="button" class="btn btn-secondary btn-xs" onclick="sms_obj.person_add(<?php echo element('sme_id', $result); ?>, '<?php echo html_escape(element('sme_name', $result)); ?>', '<?php echo element('sme_phone', $result); ?>')">추가</button></td>
				</tr>
			<?php
				}
			}
	...
	<div class="btn_list01 btn_list">
		<button type="button" class="btn btn-secondary btn-xs" onclick="sms_obj.person_multi_add()">선택추가</button>
		<button type="button" class="btn btn-secondary btn-xs" onclick="sms_obj.triggerclick('#book_group')" >그룹목록</button>
	</div>
	...
	<form name="search_form" id="sms_person_form" class="form-inline" method="get" action="<?php echo current_full_url(); ?>">
		...
		<input type="submit" value="검색" class="btn btn-secondary btn-xs" />
	</form>
</div>
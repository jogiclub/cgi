		...
		?>
			<div class="box-table-header">
				<div class="btn-group btn-group-sm" role="group">
					<a href="?" class="btn  <?php echo ( ! $this->input->get('mem_is_admin') && ! $this->input->get('mem_denied')) ? 'btn-success' : 'btn-secondary'; ?>">전체회원</a>
					<a href="?mem_is_admin=1" class="btn  <?php echo ($this->input->get('mem_is_admin')) ? 'btn-success' : 'btn-secondary'; ?>">최고관리자</a>
					<a href="?mem_denied=1" class="btn  <?php echo ($this->input->get('mem_denied')) ? 'btn-success' : 'btn-secondary'; ?>">차단회원</a>
				</div>
				<div class="btn-group btn-group-sm" role="group">
					<a href="?" class="btn  <?php echo ( ! $this->input->get('mgr_id')) ? 'btn-success' : 'btn-secondary'; ?>">전체그룹</a>
					<?php
					foreach (element('all_group', $view) as $gkey => $gval) {
					?>
						<a href="?mgr_id=<?php echo element('mgr_id', $gval); ?>" class="btn  <?php echo (element('mgr_id', $gval) === $this->input->get('mgr_id')) ? 'btn-success' : 'btn-secondary'; ?>"><?php echo element('mgr_title', $gval); ?></a>
					<?php
					}
					?>
				</div>
				...
				?>
					<div class="col-md-6"><div class="btn-group pull-right" role="group" aria-label="...">
						<button type="button" class="btn btn-outline btn-success " id="export_to_excel"><i class="bi bi-file-excel-o"></i> 엑셀 다운로드</button>
						...
						<a href="<?php echo element('write_url', $view); ?>" class="btn btn-outline btn-danger ">회원추가</a>
					</div>
				<?php
					...
					?>
						<tr>
							...
							<td>
								...
								<?php echo element('mem_is_admin', $result) ? '<span class="badge text-bg-primary">최고관리자</span>' : ''; ?>
								<?php echo element('mem_denied', $result) ? '<span class="badge text-bg-danger">차단</span>' : ''; ?>
							</td>
							...
							<?php if ($this->cbconfig->item('use_selfcert')) { ?>
								<td>
									<?php
									echo (element('selfcert_type', element('meta', $result)) === 'phone') ? '<span class="badge text-bg-success">휴대폰</span>' : '';
									echo (element('selfcert_type', element('meta', $result)) === 'ipin') ? '<span class="badge text-bg-success">IPIN</span>' : '';
									echo is_adult(element('selfcert_birthday', element('meta', $result))) ? '<span class="badge text-bg-danger">성인</span>' : '';
									?>
								</td>
							<?php } ?>
							<?php if ($this->cbconfig->item('use_sociallogin')) { ?>
							...
							<td>
								<?php echo element('mem_email_cert', $result) ? '<i class="bi bi-check-square-o"></i>' : '<i class="bi bi-square-o"></i>';; ?>
								<?php echo element('mem_open_profile', $result) ? '<i class="bi bi-check-square-o"></i>' : '<i class="bi bi-square-o"></i>';; ?>
								<?php echo element('mem_receive_email', $result) ? '<i class="bi bi-check-square-o"></i>' : '<i class="bi bi-square-o"></i>';; ?>
								<?php echo element('mem_use_note', $result) ? '<i class="bi bi-check-square-o"></i>' : '<i class="bi bi-square-o"></i>';; ?>
								<?php echo element('mem_receive_sms', $result) ? '<i class="bi bi-check-square-o"></i>' : '<i class="bi bi-square-o"></i>';; ?>
							</td>
							<td><?php echo element('mem_denied', $result) ? '<span class="badge text-bg-danger">차단</span>' : '승인'; ?></td>

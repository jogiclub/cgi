<div class="box">
	<div class="box-header">
		...
		<div class="pull-right">
			<select name="brd_id" class="form-select" onChange="location.href='<?php echo admin_url($this->pagedir . '/write_category'); ?>/' + this.value;">
		...
		<ul class="nav nav-tabs mb-3">
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write/' . element('brd_id', element('data', $view))); ?>">기본정보</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write_list/' . element('brd_id', element('data', $view))); ?>">목록페이지</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write_post/' . element('brd_id', element('data', $view))); ?>">게시물열람</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write_write/' . element('brd_id', element('data', $view))); ?>">게시물작성</a></li>
			<li class="nav-item"><a class="nav-link active" href="<?php echo admin_url($this->pagedir . '/write_category/' . element('brd_id', element('data', $view))); ?>">카테고리</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write_comment/' . element('brd_id', element('data', $view))); ?>">댓글기능</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write_general/' . element('brd_id', element('data', $view))); ?>">일반기능</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write_point/' . element('brd_id', element('data', $view))); ?>">포인트기능</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write_alarm/' . element('brd_id', element('data', $view))); ?>">메일/쪽지/문자</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write_rss/' . element('brd_id', element('data', $view))); ?>">RSS/사이트맵 설정</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write_access/' . element('brd_id', element('data', $view))); ?>">권한관리</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write_extravars/' . element('brd_id', element('data', $view))); ?>">사용자정의</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/write_admin/' . element('brd_id', element('data', $view))); ?>">게시판관리자</a></li>
		</ul>
	</div>
	<div class="box-table">
		<?php
			...
			function ca_list($p, $data)
			{
				...
				if ($p && is_array($p)) {
					foreach ($p as $result) {
						...
						if ($len) {
							$return .= '<div style="width:10px;float:left;margin-left:' . $margin . 'px;margin-right:10px;"><span class="bi bi-arrow-right"></span></div>';
						}
						...
						$return .= '
															...
															<div class="input-group" style="margin-left:0;">
																...
																<a href="javascript:;" class="btn btn-secondary btn-xs" onClick="cat_cancel(\'' . element('bca_id', $result) . '\')">취소</a>
															</div>';
						$return .= form_close();
						$return .= '				</div>
													</div>

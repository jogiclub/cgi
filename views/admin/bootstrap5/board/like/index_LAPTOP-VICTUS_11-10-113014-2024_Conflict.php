		...
		?>
			<div class="box-table-header">
				<ul class="nav nav-tabs mb-3">
					<li class="nav-item"><a class="nav-link active" href="<?php echo admin_url($this->pagedir); ?>">목록</a></li>
					<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/graph'); ?>">추천 그래프</a></li>
					<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/graph_dislike'); ?>">비추천 그래프 </a></li>
				</ul>
				<?php
				ob_start();
				?>
				<div class="col-md-6"><div class="btn-group pull-right" role="group" aria-label="...">
					<a href="<?php echo element('listall_url', $view); ?>" class="btn btn-outline btn-default ">전체목록</a>
					<button type="button" class="btn btn-outline btn-default  btn-list-delete btn-list-selected disabled" data-list-delete-url = "<?php echo element('list_delete_url', $view); ?>" >선택삭제</button>
				</div>
				<?php
				...
				<?php if (element('boardlist', $view)) { ?>
					<div class="col-md-6">
						<select name="brd_id" class="form-select" onChange="location.href='<?php echo current_url(); ?>?brd_id=' + this.value;">
				...
				<table class="table table-hover table-striped table-bordered">
					...
					?>
						<tr>
							...
							<td><a href="?brd_id=<?php echo element('brd_id', element('board', $result)); ?>"><?php echo html_escape(element('brd_name', element('board', $result))); ?></a> <a href="<?php echo goto_url(board_url(html_escape(element('brd_key', element('board', $result))))); ?>" target="_blank"><span class="bi bi-external-link"></span></a></td>
							<td><a href="?target_type=<?php echo element('target_type', $result); ?>&amp;sfield=target_id&amp;skeyword=<?php echo element('target_id', $result); ?>"><?php echo html_escape(element('origin_content', $result)); ?></a> <a href="<?php echo goto_url(element('posturl', $result)); ?>" target="_blank"><span class="bi bi-external-link"></span></a></td>
...
<select class="form-select" name="sfield" >
						...
						<span class="input-group-btn">
							<button class="btn btn-default " name="search_submit" type="submit">검색!</button>
						</span>
					</div>
				</div>
			</div>

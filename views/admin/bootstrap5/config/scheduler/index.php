<div class="box">
	<div class="box-header">
		<ul class="nav nav-tabs mb-3">
			<li class="nav-item"><a class="nav-link active" href="<?php echo admin_url($this->pagedir); ?>" onclick="return check_form_changed();">스케쥴러 등록</a></li>
			<li class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/interval'); ?>" onclick="return check_form_changed();">스케쥴 주기명</a></li>
		</ul>
	</div>
	<div class="box-table">
					...
					<div class="input-group list-group-item">
						...
						<div class="col-sm-3">
							...
							if(element('class_exists', $result)) {
								echo '<span class="badge text-bg-primary">Library 존재</span>';
							} else {
								echo '<span class="badge text-bg-danger">Library 존재않음</span>';
							}
							?>
						</div>
...
$(document).on('click', '.btn-add-rows', function() {
	$('#sortable').append(
		'<div class="input-group list-group-item"><div class="col-sm-1"><div class="bi bi-arrows" style="cursor:pointer;"></div><input type="hidden" name="key[]" /></div><div class="col-sm-3"><input type="text" class="form-control library_name" name="library_name[]" placeholder="라이브러리명" /></div><div class="col-sm-2"></div><div class="col-sm-2"></div><div class="col-sm-2"><div class="input-group input-group-sm"><select name="interval_field_name[]" class="form-control interval_field_name"><option value="">=선택=</option>' +
	<?php
	if(element('scheduler_interval', element('data', $view))) {
		foreach(element('scheduler_interval', element('data', $view)) as $skey => $svalue) {
	?>
	...
	<?php }} ?>
		'</select></div></div><div class="col-sm-1"></div><div class="col-sm-1"><button type="button" class="btn btn-outline btn-secondary btn-xs btn-delete-row" >삭제</button></div></div>');
});

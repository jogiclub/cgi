			...
			<div class="box-table-header">
				...
				?>
					<div class="col-md-6"><div class="btn-group pull-right" role="group" aria-label="...">
						...
						<button type="button" class="btn btn-outline btn-secondary  btn-list-update btn-list-selected disabled" data-list-update-url = "<?php echo element('list_update_url', $view); ?>" >선택수정</button>
						...
						<a href="<?php echo element('write_url', $view); ?>" class="btn btn-outline btn-danger ">상품추가</a>
					</div>
				<?php
					...
					?>
						<tr>
							...
							<td style="width:130px;">
								<?php foreach (element('category', $result) as $cv) { echo '<label class="badge text-bg-info">' . html_escape(element('cca_value', $cv)) . '</label> ';} ?>
								<?php if (element('cit_type1', $result)) { ?><label class="badge text-bg-danger">추천</label> <?php } ?>
								<?php if (element('cit_type2', $result)) { ?><label class="badge text-bg-warning">인기</label> <?php } ?>
								<?php if (element('cit_type3', $result)) { ?><label class="badge text-bg-default">신상품</label> <?php } ?>
								<?php if (element('cit_type4', $result)) { ?><label class="badge text-bg-primary">할인</label> <?php } ?>
							</td>
							<td>
							...
							<td class=" input-group-sm">
								<select class="form-select" name="item_layout[<?php echo element(element('primary_key', $view), $result); ?>]" >
									<?php echo element('item_layout_option', $result); ?>
								</select>
								<br />
								<select class="form-select" name="item_sidebar[<?php echo element(element('primary_key', $view), $result); ?>]">
								...
								<br />
								<select class="form-select" name="item_skin[<?php echo element(element('primary_key', $view), $result); ?>]" >
									<?php echo element('item_skin_option', $result); ?>
								</select>
							</td>
							<td class=" input-group-sm">
								<select class="form-select" name="item_mobile_layout[<?php echo element(element('primary_key', $view), $result); ?>]" >
									<?php echo element('item_mobile_layout_option', $result); ?>
								</select>
								<br />
								<select class="form-select" name="item_mobile_sidebar[<?php echo element(element('primary_key', $view), $result); ?>]">
									<option value="">기본설정따름</option>
								...
								<br />
								<select class="form-select" name="item_mobile_skin[<?php echo element(element('primary_key', $view), $result); ?>]" >

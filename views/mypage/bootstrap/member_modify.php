	...
	?>
		...
		<?php if ($this->cbconfig->item('use_selfcert') && ($this->cbconfig->item('use_selfcert_phone') OR $this->cbconfig->item('use_selfcert_ipin'))) { ?>
			<div class="input-group">
				...
				<?php } else { ?>
					회원님은 아직 본인인증을 하지 않으셨습니다. <a href="<?php echo site_url('selfcert?redirecturl=' . urlencode(current_full_url())); ?>" class="btn btn-secondary btn-sm" title="본인인증하기">본인인증 하기</a>
				<?php } ?>
				</div>
			</div>
		<?php
		...
		<div class="input-group">
			...
			<div class="col-lg-8"><a href="<?php echo site_url('membermodify/password_modify'); ?>" class="btn btn-secondary btn-sm" title="패스워드 변경">패스워드 변경</a></div>
		</div>

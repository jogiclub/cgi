...
<div class="mypage">
    <ul class="nav nav-tabs">
        ...
        <?php if ($this->cbconfig->item('use_point')) { ?>
            <li class="nav-item"><a class="nav-link active" href="<?php echo site_url('mypage/point'); ?>" title="포인트">포인트</a></li>
        <?php } ?>
	...
	<table class="table table-striped">
		...
		?>
			<tr>
				...
				<td><?php if (element('poi_point', $result) > 0) { ?><span class="badge text-bg-success">+<?php echo number_format(element('poi_point', $result)); ?></span><?php } ?></td>
				<td><?php if (element('poi_point', $result) < 0) { ?><span class="badge text-bg-danger"><?php echo number_format(element('poi_point', $result)); ?></span><?php } ?></td>
			</tr>
		<?php
			}
		}
